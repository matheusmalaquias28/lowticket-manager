...
import { ExternalLink, MoreHorizontal, ShoppingCart, Globe } from 'lucide-react'
...
export function OfferCard({ offer, onEdit, taskCount = 0, doneTaskCount = 0 }: OfferCardProps) {
  ...
  return (
    <div
      className={cn(
        'group relative p-4 rounded-xl bg-[#111118] shadow-card',
    ...
    >
      ...
      <button
        ...
        className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-[#5A5A70] hover:text-[#F0F0F8] hover:bg-[#1A1A24] transition-all"
      >
      ...
      <Link href={`/offers/${offer.id}`} className="block">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            {offer.niche && (
              <span className="text-xs text-[#5A5A70]">{offer.niche}</span>
            )}
          </div>
        </div>
