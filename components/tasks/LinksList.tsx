...
export function LinksList({ links, onChange }: LinksListProps) {
  ...
  return (
    <div className="space-y-2">
      {links.map((link, idx) => (
        <div key={idx} className="group flex items-center gap-2 p-2 rounded-lg hover:bg-[#1A1A24] transition-colors">
          ...
          {link.url && (
            <a href={link.url} target="_blank" rel="noopener noreferrer"
              className="text-[#5A5A70] hover:text-[#8B5CF6] transition-colors"
